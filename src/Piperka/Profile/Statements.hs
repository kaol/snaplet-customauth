...
profileDataFetch = statement sql encode (DE.maybeRow decodeProfile) True
  where
    ...
    sql = "SELECT uid, name, privacy=3 AS perm, privacy, writeup, \
          \COALESCE( \
          \(SELECT SUM(ord) FROM subscriptions WHERE uid=users.uid), 0) \
...
profileOwnDataFetch = statement sql encode (DE.singleRow decodeOwnProfile) True
  where
    ...
    sql = "SELECT (SELECT COUNT(*) FROM follower JOIN users AS u USING (uid) \
          \WHERE privacy > 1 AND follower.interes=users.uid) AS followers, \
          \(SELECT COUNT(*) FROM follower JOIN users AS u ON interest=u.uid \
          \WHERE privacy > 1 AND follower.uid=users.uid) AS followees, \
          \uid, name, true, privacy, writeup, \
          \COALESCE((SELECT SUM(ord) FROM subscriptions WHERE uid=users.uid), 0) \
...
profileOtherDataFetch = statement sql encode (DE.maybeRow decodeOtherProfile) True
  where
    ...
    sql = "SELECT me.privacy AS rprivacy, \
          ...
          \END AS perm, them.writeup, \
          \COALESCE((SELECT SUM(ord) FROM subscriptions WHERE uid=them.uid), 0) \
          \FROM users AS them, users AS me \
          \WHERE LOWER(them.name)=LOWER($1) AND me.uid=?"
